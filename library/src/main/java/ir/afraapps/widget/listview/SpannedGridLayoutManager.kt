...
open class SpannedGridLayoutManager(
    ...
    init {
        if (spans < 1) {
            throw RuntimeException("Invalid layout spans: $spans. Span size must be at least 1.")
        }
    }
    ...
    protected open fun measureChild(position: Int, view: View) {
        ...
        if (usedSpan > this.spans || usedSpan < 1) {
            throw RuntimeException("Invalid item span size: $usedSpan. Span size must be in the range: (1...$spans)")
        }
...
private fun Rect.isAdjacentTo(rect: Rect): Boolean {
...
private fun Rect.intersects(rect: Rect): Boolean {
